...
import com.agiledge.keocdcomet.tasks.LoginTask;
...
public class LoginActivity extends Activity implements Listener {
	...
	try{
		...
		login.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				dto.setUserName( uText.getText().toString());
				...
				dto.setCurrentActivity(LoginActivity.this);
				login.setDto(dto);
		...
		TextView textViewTripCode= (TextView) findViewById(R.id.textViewTripCode);
		textViewTripCode.setOnClickListener(new OnClickListener() {
	...
	private boolean validate(LoginDto dto)
	{
		boolean flag=true;
		 ...
		 errorText.setText("");
		if(dto.getUserName()==null||dto.getUserName().equalsIgnoreCase(""))
		{
			errorText.setText("Blank User Name !");
			flag=false;
		}else if(dto.getPassword()==null||dto.getPassword().equals(""))
		{
			errorText.setText("Blank Password !");
			flag=false;
		}
	 ...
	 private void alertExit() {
		 ...
		 alert.setPositiveButton("YES", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface arg0, int arg1) {
				...
				Toast.makeText(LoginActivity.this,"Exiting", Toast.LENGTH_SHORT);
	   ...
	   private PhoneStateListener signalStrengthListener = new PhoneStateListener(){
		...
		private int getVersionCode(Context context) {
			   ...
			   try {
			      ...
			      Log.d("***", " Version code : " + pi.versionCode + " name : " + pi.versionName );
		...
		public void update(boolean result, JSONObject obj) {
			// TODO Auto-generated method stub
			try {
				Log.d("***", " downloadSettings response in LoginActivity");
				if(result) {
					...
					if(obj.getString("ACTION" ).equalsIgnoreCase("downloadSettings")) {
						if(obj.getString("result").equalsIgnoreCase("true")) {
							...
							try {
								...
								/*if(Integer.parseInt( versionNo)> (getVersionCode(LoginActivity.this))) {
									// code to update
									
									String uri=  CometSettings. ServerAddress.substring(0,CometSettings.ServerAddress.lastIndexOf("/"));
									uri = uri.substring(0,uri.lastIndexOf("/")) + uri+ "/comet/CdCometHttps.apk";
									Log.d("***", "URI :" + uri);
									Log.d("***"," update thing is called");
...
*/							
							if(obj.getString("configurationType").equalsIgnoreCase("password type")) {
								Intent intent = new Intent(this,LoginOTPOnlyActivity .class);
								startActivity(intent);
								LoginActivity.this.finish();
							}
							} catch(Exception e) {
		...
		private BroadcastReceiver responseBcr = new BroadcastReceiver(){
			...
			public void onReceive(Context context, Intent intent) {
				...
				try {
					...
					if(nwAvailable)
					{
						TextView errorText= (TextView) LoginActivity.this.findViewById(R.id.errorLabel);
						 errorText.setText("");
					} else {
